...
# Get the number of events within 2.3--2.6 MeV window no smearing
event_energies_win = hits.groupby("event_id").energy.sum()
N_win = len(event_energies_win[ (event_energies_win >= 2.3) & ((event_energies_win <= 2.6))].index)
...
for index, e in enumerate(hits.event_id.unique()):
    ...
    dfsE1.append(ApplySmearing(hits, parts, e, 3.0))
    dfsE2.append(ApplySmearing(hits, parts, e, 2.5))
    dfsE3.append(ApplySmearing(hits, parts, e, 2.0))
    dfsE4.append(ApplySmearing(hits, parts, e, 1.5))
    dfsE5.append(ApplySmearing(hits, parts, e, 1.0))
...
dfsE1, parts1  = FilterEventE(dfsE1, parts, 3.0,  E_mean)
dfsE2, parts2  = FilterEventE(dfsE2, parts, 2.5,  E_mean)
dfsE3, parts3  = FilterEventE(dfsE3, parts, 2.0, E_mean)
dfsE4, parts4  = FilterEventE(dfsE4, parts, 1.5,  E_mean)
dfsE5, parts5  = FilterEventE(dfsE5, parts, 1.0,  E_mean)
