...
if [ "$MODE" == "CO2" ]; then
    ...
    # Because otherwise we will save no events
    sed -i "s#.*SingleParticle/min_energy.*#/Actions/DefaultEventAction/min_energy 2.458 MeV MeV#" ${CONFIG}
    sed -i "s#.*SingleParticle/max_energy.*#/Actions/DefaultEventAction/max_energy 2.458 MeV#" ${CONFIG}
    ...
    sed -i "s#.*output_file.*#/nexus/persistency/output_file ATPC_Bi_25bar#" ${CONFIG}
